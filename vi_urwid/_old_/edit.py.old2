...
class Container(urwid.Frame):
    ...
    def keypress(self, size, key):
        ...
        if key == 'esc':
            if self.esc_mode:
                self.esc_mode = False
                #self.footer_text.set_text(footer_text1)
                ...
                self.esc_mode = True
                #self.footer_text.set_text(footer_text2)
                tmp = self.footer_text.get_text()
...
class EditDisplay:
    ...
    def __init__(self, name: str, log) -> None:
        ...
        self.footer = urwid.AttrMap(urwid.Text(self.footer_data), "foot")
        #self.footer = urwid.AttrMap(urwid.Text(self.footer_text), {"foot": "GUSA"})
